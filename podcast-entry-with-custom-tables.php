<?php
/*
Plugin Name: PodcastList
Description: A plugin heavily based on Custom List Table With Db Example by Prashant Baldha that enables you to post podcasts and display them using the Jetpack plugin "Podcast Audio" gutenberg block.
Version:     1.0
Author:      Vasileios Kechagias
Author URI:  https://kechablog.ml
License:     GPL2
Podcast Entry With Custom Tables is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
any later version.
 
Podcast Entry With Custom Tables is distributed in the hope that it will be useful,
...
class Custom_Table_Example_List_Table extends WP_List_Table
{
    ...
    function column_url($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=podcasts_form&id=%s">%s</a>', $item['id'], __('Edit', 'cltd_example')),
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'url' => __('Podcast Slug', 'cltd_example'),
            'rss' => __('Podcast RSS Feed', 'cltd_example'),
        );
        return $columns;
    }
...
{
    add_menu_page(__('Podcasts', 'cltd_example'), __('Podcasts', 'cltd_example'), 'activate_plugins', 'podcasts', 'cltd_example_podcasts_page_handler', 'dashicons-megaphone');
    add_submenu_page('podcasts', __('Podcasts', 'cltd_example'), __('Podcasts', 'cltd_example'), 'activate_plugins', 'podcasts', 'cltd_example_podcasts_page_handler');
    // add new will be described in next part
    add_submenu_page('podcasts', __('Add new', 'cltd_example'), __('Add new', 'cltd_example'), 'activate_plugins', 'podcasts_form', 'cltd_example_podcasts_form_page_handler');
}
 ...
 */
function cltd_example_podcasts_page_handler()
{
    global $wpdb;
...
<div class="wrap">
    ...
    <h2><?php _e('Podcasts', 'cltd_example')?> <a class="add-new-h2"
                                 href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=podcasts_form');?>"><?php _e('Add new', 'cltd_example')?></a>
    </h2>
    <?php echo $message; ?>
 ...
 */
function cltd_example_podcasts_form_page_handler()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'cte'; // do not forget about tables prefix
    ...
    add_meta_box('podcasts_form_meta_box', 'Podcast Data', 'cltd_example_podcasts_form_meta_box_handler', 'podcast', 'normal', 'default');

    ?>
<div class="wrap">
    <div class="icon32 icon32-posts-post" id="icon-edit"><br></div>
    <h2><?php _e('Podcast', 'cltd_example')?> <a class="add-new-h2"
                                href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=podcasts');?>"><?php _e('Back to list', 'cltd_example')?></a>
    </h2>
        ...
        <div class="metabox-holder" id="poststuff">
            <div id="post-body">
                <div id="post-body-content">
                    ...
                    <?php do_meta_boxes('podcast', 'normal', $item); ?>
 ...
 * @param $item
 */
function cltd_example_podcasts_form_meta_box_handler($item)
{
    ?>
...
<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
    ...
    <tr class="form-field">
        <th valign="top" scope="row">
            <label for="url"><?php _e('Podcast Slug', 'cltd_example')?></label>
        </th>
        <td>
            <input id="url" name="url" type="text" style="width: 95%" value="<?php echo esc_attr($item['url'])?>"
                   size="50" class="code" placeholder="<?php _e('Podcast Slug', 'cltd_example')?>" required>
        </td>
    </tr>
    <tr class="form-field">
        <th valign="top" scope="row">
            <label for="rss"><?php _e('Podcast RSS Feed', 'cltd_example')?></label>
        </th>
        <td>
            <input id="rss" name="rss" type="text" style="width: 95%" value="<?php echo esc_attr($item['rss'])?>"
                   size="50" class="code" placeholder="<?php _e('Podcast RSS Feed', 'cltd_example')?>" required>
        </td>
    </tr>
    </tbody>
</table>
...
function cltd_example_validate_person($item)
{
    ...
    if (empty($item['url'])) $messages[] = __('Podcast URL is required', 'cltd_example');
    if (empty($item['rss'])) $messages[] = __('Podcast RSS is required', 'cltd_example');
...
{
    if ( is_page( 'podcast' ) ) {
        $page_template = dirname( __FILE__ ) . '/templates/single-podcast.php';
    }
